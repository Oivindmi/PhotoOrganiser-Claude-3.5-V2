...
class MetadataExtractor:
    ...
    def process_batch_with_exiftool(self, file_paths):
        try:
            ...
            cmd = ['exiftool', '-j', '-a', '-G', '-time:all', '-charset', 'filename=utf8', '-@', arg_file]
    ...
    def extract_relevant_metadata(self, metadata):
        ...
        for item in metadata:
            file_metadata = {
                ...
                'camera_model': item.get('EXIF:Model') or item.get('QuickTime:Model') or item.get('File:FileType'),
                'file_size': float(item.get('File:FileSize', 0)) / (1024 * 1024),  # Convert to MB
            }

            # Extract all time-related fields
            for key, value in item.items():
                if 'Date' in key or 'Time' in key:
                    file_metadata[key.replace(':', '_')] = value
    ...
    def extract_relevant_metadata(self, metadata):
        ...
        for item in metadata:
            ...
            file_metadata = {
                ...
                'file_info': None,  # To be filled by the caller
                'camera_model': item.get('EXIF:Model') or item.get('QuickTime:Model') or item.get('File:FileType'),
                'file_size': os.path.getsize(file_path) / (1024 * 1024) if os.path.exists(file_path) else 0,  # Get actual file size in MB
            }
