...
class TimeSynchronizer(QObject):
    ...
    def update_group_times(self, session, base_group_id, compare_group_id, base_file, compare_file, left_field,
                           right_field, choice):
        ...
        if base_time and compare_time:
            ...
            if base_group_id not in self.evaluated_groups:
                self.update_group(session, base_group_id, left_field, base_time)
                self.evaluated_groups.add(base_group_id)

            if compare_group_id not in self.evaluated_groups:
                self.update_group(session, compare_group_id, right_field, compare_time)
                self.evaluated_groups.add(compare_group_id)

            if choice == 'left':
                self.apply_time_delta(session, compare_group_id, time_delta)
            else:
                self.apply_time_delta(session, base_group_id, -time_delta)
    ...
    def _find_datetime_in_metadata(self, metadata, field):
        # First, try to get the value directly using the field name
        value = metadata.get(field)
        if value:
            return self._parse_datetime(value)

        # If not found, search for a key that contains the field name
        for key, val in metadata.items():
            if field.lower() in key.lower():
                return self._parse_datetime(val)

        # If still not found, log an error and return None
        logger.error(f"Could not find datetime for field: {field} in metadata")
        return None

    def update_group(self, session, group_id, field, reference_time):
        ...
        for file in files:
            ...
            original_time = self._parse_datetime(file_metadata.get(field))
            if original_time:
                file.correct_time = reference_time
                file.original_time_field = field
                logger.info(
